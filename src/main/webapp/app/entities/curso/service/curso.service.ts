...
export class CursoService {
  ...
  protected convertDateFromClient(curso: ICurso): ICurso {
    return Object.assign({}, curso, {
      periodo: curso.periodo?.isValid() ? curso.periodo.format(DATE_FORMAT) : undefined,
      criado: curso.criado?.isValid() ? curso.criado.toJSON() : undefined,
    });
  }

  protected convertDateFromServer(res: EntityResponseType): EntityResponseType {
    if (res.body) {
      res.body.periodo = res.body.periodo ? dayjs(res.body.periodo) : undefined;
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((curso: ICurso) => {
        curso.periodo = curso.periodo ? dayjs(curso.periodo) : undefined;
        curso.criado = curso.criado ? dayjs(curso.criado) : undefined;
      });
    }
    return res;
  }
